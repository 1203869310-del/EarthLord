...
struct DeviceManagementView: View {
    ...
    var body: some View {
        ...
        .alert("设备未解锁", isPresented: $showUnlockAlert) {
            ...
            if let device = selectedDeviceForUnlock,
               let requirement = device.unlockRequirement {
                Text(requirement)
            }
        }
    }
        ...
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("覆盖范围: \(device.deviceType.rangeText)")
                    ...
                    .foregroundColor(ApocalypseTheme.textSecondary)
                HStack(spacing: 4) {
                    Image(systemName: device.deviceType.canSend
    ...
    private func handleTap(_ deviceType: DeviceType, _ isUnlocked: Bool, _ isCurrent: Bool) {
        ...
        Task { await communicationManager.switchDevice(userId: userId, deviceType: deviceType) }
    }
}
